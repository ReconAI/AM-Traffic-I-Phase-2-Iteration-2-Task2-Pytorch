...
3. In the root folder create the output directory **output_road_torch**.
4. From the root folder execute:
    ```sh
   python3 predict_images_torch.py --model ./models/road_model.pt --weights  ./models/weights_road.pth --input ./WeatherRoad_EvaluationDataset/road --output ./output_road_torch --labels ./road_labels.json
    ```
    Where:
...
4. Check annotated images in /output_road_torch folder and network metrics in the terminal window.
### Results
...
```sh
python3 convert_pt_to_keras.py --model ./models/torch/road_model.pt --weights ./models/torch/weights_road.pth --keras_path ./models/torch_keras/
```
Where:
...
```sh
python3 convert_keras_to_trt.py --trt_path ./models/torch_trt --model ./models/torch_keras/road_model.h5 --output_node  test_output/BiasAdd
```
Where:
...
6. From the folder where files are uploaded execute:
   ```sh
   python3 predict_trt.py --input_node test_input --output_node test_output/BiasAdd --path r_pt_32_tensorrt_model.pb --img_path img0.jpg --labels road_labels.json --shape_size 224 --time true
   ```
   ...
   ## Results
   For model with 'Foating Point Precision' equal to 32 the **average(sec/frame)** varies from 0.18 to 0.19 and the **fps** varies from 5.28 to 5.71.
   
   For model with 'Foating Point Precision' equal to 16 the **average(sec/frame)** varies from 0.17 to 0.19 and the **fps** varies from 5.27 to 5.61.
